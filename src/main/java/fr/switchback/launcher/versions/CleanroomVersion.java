...
public class CleanroomVersion extends AbstractModLoaderVersion {
    ...
    public CleanroomVersion(String modLoaderVersion, List<Mod> mods, List<CurseFileInfo> curseMods, List<ModrinthVersionInfo> modrinthMods, ModFileDeleter fileDeleter, CurseModPackInfo curseModPackInfo, ModrinthModPackInfo modrinthModPackInfo) {
    ...
    public boolean isModLoaderAlreadyInstalled(@NotNull Path installDir) {
        ...
        try {
            final JsonObject object = JsonParser.parseReader(Files.newBufferedReader(versionJsonFile)).getAsJsonObject();
            return ModLoaderUtils.parseNewVersionInfo(installDir, object).stream().allMatch(ParsedLibrary::isInstalled);
        } catch (Exception e) {
    ...
    public void install(@NotNull Path installDir) throws Exception {
        ...
        final String installerUrl = String.format("https://repo.cleanroommc.com/releases/com/cleanroommc/cleanroom/%s/cleanroom-%s-installer.jar", this.modLoaderVersion, this.modLoaderVersion);
        ...
        IOUtils.download(this.logger, new URI(installerUrl).toURL(), installerFile);
        ...
        try (final FileSystem zipFs = FileSystems.newFileSystem(uri, new HashMap<>())) {
            ...
            ModLoaderUtils.parseNewVersionInfo(installDir, JsonParser.parseReader(Files.newBufferedReader(versionFile)).getAsJsonObject()).stream().filter(parsedLibrary -> !parsedLibrary.isInstalled()).forEach(parsedLibrary -> {
